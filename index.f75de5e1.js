...
tableHead.addEventListener("click", (e)=>{
    ...
    if (currentTarget.tagName === "TH") {
        ...
        if (lastIndex === index) sortDirection = sortDirection === "asc" ? "desc" : "asc";
        ...
        array.sort((a, b)=>{
            ...
            if (!isNaN(aValue) && !isNaN(bValue)) return direction * (aValue - bValue);
...
save.type = "submit";
form.append(nameLabel, positionLabel, citySelectLabel, ageLabel, salaryLabel, save);
table.after(form);
...
form.addEventListener("submit", (e)=>{
    e.preventDefault();
    if (nameInput.value.length < 4) return showNotification("Wrong", "error");
    if (ageInput.value < 18 || ageInput.value > 90) return showNotification("Wrong", "error");
    if (positionInput.value === "") return showNotification("Wrong", "error");
    if (salaryInput.value === "") return showNotification("Wrong", "error");
...
tableBody.addEventListener("dblclick", (e)=>{
    ...
    if (!td) return;
    if (tableBody.querySelector(".cell-input")) return;
    ...
    input.addEventListener("keydown", (ev)=>{
        if (ev.key === "Enter") td.textContent = input.value || initialValue;
    });
